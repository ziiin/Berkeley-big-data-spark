...
def getCountsAndAverages(IDandRatingsTuple):
    ...
    #x= (IDandRatingsTuple[0],(count(IDandRatingsTuple[1],sum(IDandRatingsTuple[1])/count(IDandRatingsTuple[1])))
    return (IDandRatingsTuple[0],(count, sum2/ float(count)))
...
movie_id= moviesRDD.collectAsMap()
movieNameWithAvgRatingsRDD = movieIDsWithAvgRatingsRDD.map(lambda (x,y) : (y[1],movie_id[x],y[0]))
...
def computeError(predictedRDD, actualRDD):
    ...
    squaredErrorsRDD = (predictedReformattedRDD
                        ...
                        .map(lambda (x,y) : (x,math.pow((y[0] - y[1]),2)))
                       )
    #print squaredErrorsRDD.take(2)

    #Compute the total squared error - do not use collect()
    #print squaredErrorsRDD.map(lambda (x,y) : y).reduce(lambda x,y : x+y)
    totalError = squaredErrorsRDD.map(lambda (x,y) : y).reduce(lambda x,y : x+y)
...
#computeError(testPredicted, testActual)
...
trainingAvgRating = trainingRDD.map(lambda (x,y,z) : z).reduce(lambda x,y : x+y) / trainingRDD.count()
...
# TODO: Replace <FILL IN> with appropriate code
myRatingsModel = ALS.train(trainingWithMyRatingsRDD, 8, seed=seed, iterations=iterations,
                      lambda_=regularizationParameter)
...
my_rate = map(lambda (x,y,z) : y, myRatedMovies)

myUnratedMoviesRDD = (moviesRDD
                      .map(lambda (x,y) :(myUserID,x))).filter(lambda (a,b) : b not in my_rate )

                      
#                     .filter(lambda (a,b) : (a,b) if b not in my_rate))
...
movie_map = moviesRDD.collectAsMap()
ratingsWithNamesRDD = (predictedWithCountsRDD.map(lambda (x,y) : (y[0],movie_map[x],y[1])))
