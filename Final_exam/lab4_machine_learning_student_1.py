...
def getCountsAndAverages(IDandRatingsTuple):
    ...
    """
    key = IDandRatingsTuple[0]
    v1 = len(IDandRatingsTuple[1])
    v2 = sum(IDandRatingsTuple[1]) / float(len(IDandRatingsTuple[1]) )
    return (key, (v1,v2))
...
movieNameWithAvgRatingsRDD = (moviesRDD
                              ...
                              .map(lambda (movId, (movName,(numRating,avgRating))): (avgRating,movName,numRating))
                              )
# .map(lambda (movId, (movName,(numRating,avgRating))): (avgRating,movName,numRating))
#print movieNameWithAvgRatingsRDD.take(2)
...
movieLimitedAndSortedByRatingRDD = (movieNameWithAvgRatingsRDD
                                    .filter(lambda (avgRating, movName, numRating): numRating > 500)
