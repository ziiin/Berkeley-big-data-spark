...
def simpleTokenize(string1):
    ...
    return filter (lambda x: len(x)!= 0, re.split(split_regex, string1.lower()))
    #return filter(None, re.split(split_regex, string1.lower()))
...
def countTokens(vendorRDD):
    ...
    """
    return vendorRDD.map(lambda(a,b):len(b)).reduce(lambda a,b : a+b)
...
def findBiggestRecord(vendorRDD):
    ...
    return [vendorRDD.reduce(getMax)]
    '''
    return vendorRDD.takeOrdered(1, lambda a: -1 *len(a[1]))
...
def idfs(corpus):
    ...
    uniqueTokens = corpus.flatMapValues(lambda x: set(x))
...
avgSimDups = trueDupsRDD.map(lambda (x,y):y[0]).reduce(add) / trueDupsCount


dupKeys = trueDupsRDD.keys().collect()


nonDupsRDD = (sims
              .filter(lambda(x,y): x not in dupKeys))
avgSimNon = nonDupsRDD.map(lambda (x,y):y).reduce(add) / nonDupsRDD.count()
...
amazonWeightsRDD = ( amazonFullRecToToken
                    .map(lambda (x,y): (x, tfidf(y, idfsFullBroadcast.value))))
...
commonTokens = (amazonInvPairsRDD
                ...
                .map(lambda (a,b): (a, b.split(" ")))
                .cache())

print 'Ajha: %s' % commonTokens.take(10)
...
def fastCosineSimilarity(record):
    ...
    computes the sum 
    ( for each of the tokens in the token list)
         of the products 
               of the Amazon weight for the token times 
               the Google weight for the token
    """
    ID = record[0][0]
    URL = record[0][1]
    
    amazonRec = amazonWeightsBroadcast.value[ID]
    googleRec = googleWeightsBroadcast.value[URL]
    
    amazonNorm = amazonNormsBroadcast.value[ID]
    googleNorm = googleNormsBroadcast.value[URL]
    
    tokens = record[1]
    
    s = 0
    for token in tokens:
        s +=  amazonRec[token] * googleRec[token]
    value = float(s) / (amazonNorm * googleNorm) 
    key = (ID, URL)
    return (key, value)
