...
not200 = ( access_logs
          ...
          .filter(lambda (end_point, res_code): res_code!= 200)
          )


endpointCountPairTuple = ( not200
                          .map(lambda ep: (ep[0],1))
                          )
...
#ajha: status: need unique hosts not all

dayToHostPairTuple = ( access_logs
                      .map (lambda log: (log.date_time.day, log.host))
                      )

#sample = dayToHostPairTuple.take(2)
#print 'Sample %s' %sample

dayGroupedHosts = dayToHostPairTuple.groupByKey()

#sample = dayGroupedHosts.take(10)
#print 'Sample %s' %sample

dayHostCount = (dayGroupedHosts
                .mapValues(lambda host_list: set(host_list))
                )
#sample = dayHostCount.take(10)
#print 'Sample %s' %sample

dailyHosts = (dayHostCount
              .mapValues(lambda host_list: len(host_list))
              .sortByKey()
              .cache()
             )
...
#sample = sortedByDay.take(4)
#print "Sample : %s" % sample


avgDailyReqPerHost = (sortedByDay
                      .map(lambda (days, hosts): (days, len(hosts)/ len(set(hosts))))
                     .cache())
...
badUniqueEndpoints = badEndpoints.keys()
...
badEndpointsTop20 = badEndpointsSum.takeOrdered (20, lambda ep: -1 * ep[1])
...
errHostsTop25 = errHostsSum.takeOrdered (25, lambda hp: -1 * hp[1])
